...
parser.add_argument(
    "--map",
    default="/bigdata03/traffic4cast2020/processed_data",
    type=str,
)
...
def transform(x):
    new_x = np.zeros([495, 436, 9],dtype=np.float32)
    for i in range(9):
        new_x[:,:,i] = transform_map[i][x[:,:,i]]
    return new_x

if __name__ == "__main__":
    args = parser.parse_args()
    args.city = args.city.upper()
    transform_map = np.load(os.path.join(args.map,f"{args.city}_value_map.npz"))['array']
    transform_map = transform_map.astype(np.float32)
    ...
    env = lmdb.open(os.path.join(args.ouput_dir,f"{args.city}_GK"), map_size=args.max_size)
